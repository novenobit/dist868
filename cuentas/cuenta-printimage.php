...
{ $sqlCuenta1=mysqli_query($conex1,"select * from cuentas1 where id_cuenta='$id_cuenta'"); }
if(isset($rand_num) and $rand_num<>"")
{ $sqlCuenta1=mysqli_query($conex1,"select * from cuentas1 where rand_num='$rand_num'"); }
$numFilas=mysqli_num_rows($sqlCuenta1);

if($numFilas>0)
{
 $cuentaData=mysqli_fetch_array($sqlCuenta1);
...
<table style="width:600px;">
 <thead>
  <tr>
   <th class='grey center aligned'>Img</th>
...
<?php
 ...
 $Ttotal=0;
 $numFilas=0;
 $sqlcuenta2=mysqli_query($conex1,"select * from cuentas2 where id_cuenta='$id_cuenta' order by empaque desc");
 $numFilas=mysqli_num_rows($sqlcuenta2);
 if($numFilas>0)
 {
  while($cuenta2Data=mysqli_fetch_array($sqlcuenta2))
  {
    $id_cuenta=$cuenta2Data['id_cuenta'];
    $id_cuenta=$cuenta2Data['id_cuenta'];
    $id_producto=$cuenta2Data['id_producto'];
    $cantidad=$cuenta2Data['cantidad'];
    //$empaque=$cuenta2Data['empaque'];
    $precio_producto=$cuenta2Data['precio_producto'];
    //$precio=$cuenta2Data['precio'];
// Find Oroducto
    $sqlPro=mysqli_query($conex1,"select id_producto,cod_producto,cod_categoria,cod_subcategoria,nom_producto,precio1_producto,precio2_producto,precio3_producto,precio4_producto,nom_unidad,empaque,foto_producto from productos where id_producto='$id_producto'");
...
// Check Price
    ...
    if($proData['empaque']<>$cuenta2Data['empaque'])
    {
      $empaque="";
      //$Tcant="";
    }
    ...
    echo "<tr>
     <td class='center aligned'>";
      ...
      { echo "<img class='ui image' src='$dirRoot"."imagenes/productos/$fotoProducto' style='height:60px;'>"; }
     echo "</td>
 ...
 if($reg==0)
 {
    echo "<tr><td class='center aligned' colspan='7' style='padding: 60px;'>
