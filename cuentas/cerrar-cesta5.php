...
// -----------------------------------------------------
    if(isset($numfactura) and $numfactura<>"")
    {
        ...
        while($pagosData=mysqli_fetch_array($sqlPagos))
        {
          ...
          if($montopago>0)
          {
            ...
            { $id_formapago=""; }
            $numFilas=0;
            $sql=mysqli_query($conex1, "select id_cuenta from respaldo_pagos where numfactura='$numfactura' or cid_cliente='$cid_cliente' and montopago='$montopago' and dia='$dia' and mes='$mes'");
        ...
        while($cestaData=mysqli_fetch_array($sqlCesta))
        {
          ...
          if($numFilas==0)
          {
            $sql2="insert into respaldo_cesta(numfactura, cid_cliente, empleado, id_cuenta,  cod_producto,codigo_barra, cantidad, precio1_producto, dia, mes, ano, submitted,  anulada)
            values('$numfactura', '$cid_cliente','$usuario', '$id_cuenta', '$cod_producto', '$codigo_barra', '$Tcantidad', '$precio_producto',  '$dia', '$mes', '$ano', '$submitted', '$anulada')";
            // echo "<br>$sql2";
    ...
    echo "<html><meta http-equiv=refresh content=0;url=cerrar-cesta6.php?id=$iduser&sistema=$sistema>";
?>
