...
<h2>Compare Data de Produtos con ProductosLimpio</h2>
<div class="ui grid">
<?php
...
// ----------------------------------------
 ...
 if($cod_producto=="")
 {
  echo "<div class='sixteen wide column'>";
   ...
   $query2="update productos set cod_producto='$codigo_barra' where id_producto='$id_producto'";
...
// ----------------------------------------
 $TempNom_producto=str_replace("'", "", $nom_producto);
// ----------------------------------------

 $numFilas1=0;
 //echo "<br>select * from productoslimpio where cod_producto='$cod_producto' or codigo_barra='$codigo_barra' or nom_producto='$TempNom_producto'";

 $sql2=mysqli_query($conex1, "select * from productoslimpio where cod_producto='$cod_producto' or codigo_barra='$codigo_barra' or nom_producto='$TempNom_producto'");
...
// Data 1----------------------------------
  echo "<div class='sixteen wide column'>
    <strong>$id_producto) $nom_producto - $codigo_barra</strong> - (".number_format($precio1_producto,2,',', '.').") - (".number_format($precio2_producto,2,',', '.').") - (".number_format($precio3_producto,2,',', '.').") - (".number_format($precio4_producto,2,',', '.').") - $foto_producto";
  while($proData2=mysqli_fetch_array($sql2))
  {
   $Tid_producto=$proData2['id_producto'];
   $Tcod_producto=$proData2['cod_producto'];
   $Tcodigo_barra=$proData2['codigo_barra'];
   $Tcod_upcean=$proData2['cod_upcean'];
   $Tcod_categoria=$proData2['cod_categoria'];
   $Tcod_subcategoria=$proData2['cod_subcategoria'];
   $Tnom_producto=$proData2['nom_producto'];
   $Tpro_brevedato=$proData2['pro_brevedato'];
   $Tdatos_producto=$proData2['datos_producto'];
   $Tcod_proveedor=$proData2['cod_proveedor'];
   $Tpaisorigen=$proData2['paisorigen'];
   $Tbrand_marca=$proData2['brand_marca'];
   $Tprecio_compra=$proData2['precio_compra'];
   $Tprecio1_producto=$proData2['precio1_producto'];
   $Tprecio2_producto=$proData2['precio2_producto'];
   $Tprecio3_producto=$proData2['precio3_producto'];
   $Tprecio4_producto=$proData2['precio4_producto'];
   $Tnom_unidad=$proData2['nom_unidad'];
   $Tempaque=$proData2['empaque'];
   $Tstock_actual=$proData2['stock_actual'];
   $Tfoto_producto=$proData2['foto_producto'];
   $Tespecial=$proData2['especial'];
   $Tcambia_precio=$proData2['cambia_precio'];
   $Tsubmitted=$proData2['submitted'];
   $Testado=$proData2['estado'];
   $Tactivo=$proData2['activo'];
// Data 2----------------------------------
...
echo "<br><span class='font-red'>".$query2."</span>";
   ...
   if($Tpro_brevedato<>"")
   {
    if($pro_brevedato<>"")
    {
     ...
     { $pro_brevedato="$pro_brevedato - $Tpro_brevedato"; }
    }
    if($pro_brevedato=="")
