...
//  -----------------------------------------
?>
    <h2 class="menu-label">Clientes</h2>
<?php
     $numFilas=0;
     $sqlNumVentas=mysqli_query($conex1,"select id_cliente from clientes limit 2");
     $numFilas=mysqli_num_rows($sqlNumVentas);
     if($numFilas>0)
     {
?>
    <h2 class="menu-label">Listado de Clientes</h2>
    <table class="ui unstackable celled long scrolling table">
     ...
     <?php
      $reg=0;
      $num=1;
      $sqlCliente=mysqli_query($conex1,"select id_cliente,cid_cliente,nom_cliente,tel1_cliente from clientes");
      while($clienteData=mysqli_fetch_array($sqlCliente))
      {
       ...
       echo "<tr><td class='center aligned'>$num</td>
        ...
        <td><a href='abrir-cuenta2.php?id=$id'>$nom_cliente</a></td>
